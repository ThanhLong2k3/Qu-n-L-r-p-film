  ...
  const scheduleData = {
    locations: showtimes
      ? showtimes.map((showtime) => ({
          showtimeId: showtime._id, // Thêm ID của suất chiếu
          city: showtime.theater.name, // Lấy tên thành phố từ dữ liệu showtime
          date: dayjs(showtime.time).format('YYYY-MM-DD'),
          name: showtime.theater.name, // Lấy tên rạp
          times: [{ time: dayjs(showtime.time).format('HH:mm'), id: showtime._id }], // Lấy giờ chiếu kèm ID
        }))
      : [],
  };
